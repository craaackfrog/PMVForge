...
export default function GeneratePage() {
  ...
  function applyPreset(preset) {
    if (!preset) return
    playClick()
    const g = preset.generate || {}
    const lib = preset.library || {}
    setActivePresetId(preset.id)
    setEditPresetName(preset.name || '')
    setEditPresetDesc(preset.description || '')
    setForm((f) => ({ ...f, ...g }))
    setDraft((d) => {
      const next = { ...d, form: { ...(d.form || {}), ...g } }
      if (lib.tags) next.libraryTags = [...lib.tags]
      if (lib.tag_mode) next.libraryTagMode = lib.tag_mode
      if (lib.min_heat != null) next.libraryMinHeat = lib.min_heat
      if (lib.prefer_library && next.clipMode !== 'library') {
        // hint user toward library mode without forcing if no library selected
        // keep current clipMode
      }
      return next
    })
    // stash CH defaults for Cock Hero page
    try {
      localStorage.setItem(
        'pmvforge:ch-preset',
        JSON.stringify({ presetId: preset.id, name: preset.name, cockhero: preset.cockhero || {} })
      )
    } catch (_) {}
  }
  ...
  async function savePresetMeta() {
    ...
    setPresetBusy(true)
    try {
      const res = await fetch(`/api/presets/${encodeURIComponent(activePresetId)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: editPresetName.trim() || 'Untitled',
          description: editPresetDesc,
        }),
      })
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).detail || 'Update failed')
      const updated = await res.json()
      setPresets((list) => {
        const rest = list.filter((x) => x.id !== activePresetId && x.id !== updated.id)
        // if builtin was promoted, switch id
        return [updated, ...rest.filter((x) => x.id !== updated.id)]
      })
      setActivePresetId(updated.id)
      setEditPresetName(updated.name || '')
      setEditPresetDesc(updated.description || '')
  ...
  async function saveCurrentAsPreset() {
    const name = saveName.trim() || 'My mode'
    setPresetBusy(true)
    try {
      const body = {
        name,
        description: 'Saved from Generate',
        generate: { ...form },
        library: {
          tags: libraryTags,
          tag_mode: libraryTagMode,
          min_heat: libraryMinHeat,
          prefer_library: clipMode === 'library',
        },
        cockhero: (() => {
          try {
            const raw = localStorage.getItem('pmvforge:ch-preset')
            return raw ? JSON.parse(raw).cockhero || {} : {}
          } catch {
            return {}
          }
        })(),
      ...
      <section className="rounded-lg border border-border bg-card p-5 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-serif text-lg">Mode</h2>
          </div>
          <div className="flex items-center gap-2">
            <button
              type="button"
              ...
              onClick={() => {
                ...
                if (p) {
                  ...
                  setEditPresetDesc(p.description || '')
                }
                setShowEditPreset((v) => !v)
              }}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-secondary text-sm hover:bg-accent disabled:opacity-40"
            >
              Edit
            </button>
            <button
              type="button"
              onClick={() => { playClick(); setShowSavePreset((v) => !v) }}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-secondary text-sm hover:bg-accent"
            >
              <Bookmark size={14} />
              Save current
            </button>
        ...
        <p className="text-xs text-muted-foreground">
          Bundles aspect, clip order, library tags/heat, and Cock Hero defaults. Edit name & description anytime.
        </p>
        <select
          ...
          className="w-full px-3 py-2.5 rounded-md bg-secondary border border-border text-sm"
        >
          <option value="">Custom (no mode)</option>
          {presets.map((p) => (
            <option key={p.id} value={p.id}>
              {p.name}{p.builtin ? ' · built-in' : ''}{p.description ? ` — ${p.description}` : ''}
            </option>
          ))}
        </select>
        {activePresetId && !showEditPreset && (
          <p className="text-xs text-muted-foreground">
            {presets.find((x) => x.id === activePresetId)?.description || 'No description'}
          </p>
        )}
        {showEditPreset && activePresetId && (
          <div className="rounded-md border border-border bg-secondary/30 p-3 space-y-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-primary text-primary-foreground text-sm font-medium disabled:opacity-50"
              >
                {presetBusy ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
                Save name & description
              </button>
              <button
                type="button"
                onClick={() => setShowEditPreset(false)}
        ...
        {showSavePreset && (
          <div className="flex flex-wrap items-center gap-2 pt-1">
            <input
              ...
              className="px-3 py-1.5 rounded-md bg-secondary border border-border text-sm flex-1 min-w-[160px]"
            />
            <button
              type="button"
              disabled={presetBusy}
              onClick={saveCurrentAsPreset}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-pink-500 text-white text-sm font-medium disabled:opacity-50"
            >
              {presetBusy ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
              Save mode
            </button>
